...
/**
 * Gate3-P5：:player → :shared 的 PiP 播放器注册桥。
 *
 * 依赖方向是 :shared 消费 :player，引擎（在 :player）拿不到 :shared 的
 * `IosPipPlayerHolder`（P1 搬迁后 IosAVPlaybackEngine 直接引用它是遗留的
 * 反向依赖，本地离线编 iOS 未暴露）。改为：引擎只调本桥，
