 ...
 *
 * ⚠️ 目前**只有桌面**真的走这条 mpv shader 路（Android 走 mediamp-exo 的
 * `setVideoEffects`，代码见 `ExoSuperResolution`；iOS 无对等能力，引擎如实给 null）。
 * 档位常量三端共用（见 `video.contract.VideoEnhancementLevels`，UI 菜单与落盘的取值域）。
 */
object MpvShaders {
 ...
 */
expect suspend fun materializeMpvShaders(level: Int): String?
