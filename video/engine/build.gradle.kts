// :video:engine —— 三端 mediamp 播放实现 + 状态派生（由原 :player 改名而来）。
//
// 装载：PlaybackEngine 接口 + 三端引擎 + 共享基类 + 纯弹幕解算 + 超分胶水
// （MpvShaders/ExoSuperResolution）。**不含 Compose UI**（渲染面在 :video:surface，
// 弹幕绘制层在 :video:ui；分层由 ModuleLayeringTest 的 T9 守）。
// 包名不变（仍 lovehan1me.*），只换模块。target / 源集层级 / 单测宿主由
// han1me-kmp-library 约定插件提供，本文件只保留模块特有配置。
...
kotlin {
    sourceSets {
        commonMain.dependencies {
            // 播放契约层。api 导出：本模块的公开签名（PlaybackEngineState 等）
            // 已经用上契约层的类型，不导出 :shared 编译期看不到。
            ...
            // 直接依赖 + ktor 带入的 io 抬到 0.8.0 / 0.9.1；:video:engine 无 ktor，
